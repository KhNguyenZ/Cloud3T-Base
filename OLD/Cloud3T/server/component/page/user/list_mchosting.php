...
<title><?= $setting['Title'] ?> - Trang Quản Lí Minecraft Hosting Đã Mua</title>
                ...
                <div class="row">
                    <div class="card-header">
                        <div class="card-title">
                            <h3>Danh sách Minecraft Hosting đã mua</h3>
                        </div>
                    </div>
                    <div class="table-responsive">
                        <table class="table align-middle table-row-bordered table-row-solid gy-4 gs-9">
                            ...
                            <tbody class="fw-6 fw-semibold text-gray-600">
                                <?php foreach ($KNCMS->get_list("SELECT * FROM `user_mc` WHERE `uid` = " . $KNCMS->getUser($_SESSION['username'])['id']) as $hosting_sv) { 
                                    $mcplan = GetMCPlan('id', $hosting_sv['plan_id']);
                                    ?>
                                    <tr>
                                        <td>
                                            <a href="#" class="text-hover-primary text-gray-600"><?= $hosting_sv['name'] ?></a>
                                        </td>
                                        <td>
                                            <?= $hosting_sv['createdtime'] ?>
                                        </td>
                                        <td>
                                            <?= $hosting_sv['expiredtime'] ?? 'Đang tạo hosting' ?>
                                        </td>
                                        <td>
                                            <?= $mcplan['package_name'] ?>
                                        </td>
                                        ...
                                        <td>
                                            <a class="btn btn-success my-2 btn-lg" href="<?= hUrl('User/Manage/MCHosting/' . $hosting_sv['id']) ?>">

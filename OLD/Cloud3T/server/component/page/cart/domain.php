...
if (isset($_GET['hid'])) {
    if (check_rows('hosting', 'id', $_GET['hid'])) {
        $hid = $_GET['hid'];
        $hinfo = $KNCMS->query("SELECT * FROM `domain` WHERE `id` = '$hid'")->fetch_array();
    } else $KNCMS->msg_warning("Không tìm thấy đơn hàng này !", hUrl('Home'), 1000);
}
?>

<title><?= $setting['Title'] ?> - Trang Thanh Toán Domain</title>
                ...
                <div class="row">
                    <div class="col-lg-7">
                        ...
                        <div class="row">
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title"><?= $hinfo['domain'] ?></h5>
                                    <div class="fs-5 mb-2">
                                        <span class="badge badge-light-success fw-bold"><?= $KNCMS->format_cash($hinfo['price']) ?>vnđ</span>
                                        <span class="text-gray-600 fw-semibold">/ 1 năm</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <form method="post">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <div class="fs-5 mb-2">
                                            <span class="text-gray-800 fw-bold me-1">Chỉ nhập tên của miền (VD: cloud247,vpsgiare,facebook,domaingiare,...)</span>
                                        </div>
...
<?php require_once $client_controller . '/domain/buy.php' ?>
</div>
